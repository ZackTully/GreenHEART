...
def make_GreenHeartSimulationConfig():
    ...
    fname_greenheart_config = str(
        config_root / "plant/greenheart_config_onshore_mn.yaml"
    )
...
def test_step_control_model_runs():
    ...
    # Should not raise
    ctrl.control_model.step_control_model(
...
def test_collect_system_matrices_dimensions():
    ...
    assert ctrl.control_model.block_ss.shape == (25, 20)

    block_ss = np.load(Path(__file__).parent / "dispatch_inputs/block_ss.npy")


    assert np.all(np.isclose(ctrl.control_model.block_ss, block_ss, atol=1e-4))
...
def test_compute_trajectory():
    ...
    x0 = np.array([1800000.0, 4114340.6564, 11250000.0, 783000.0])
    forecast = np.array(
        [94841.8283, 81526.1966, 118532.1439, 111062.7366, 235747.4349, 234696.2002]
    )

    uct, usp, curtail, grid, obj_values_uw = ctrl.compute_trajectory(
        x0, forecast, ret_obj=True
    )

    expected_obj_vals = {
        "output_tracking": 3.97393117528372e-05,
        "gridcurtail": 4.915694405667124e-08,
        "bes_simultaneous": -2.4360214225290797e-13,
        "tes_simultaneous": 0.04816952636539321,
        "h2s_simultaneous": -2.9642322199173837e-12,
        "bes_state": 54252866095.87565,
        "tes_state": 26212807767874.977,
        "h2s_state": 362485325710.6407,
        "bes_soc_state": 0.013563216523968916,
        "tes_soc_state": 0.6248051257477527,
        "h2s_soc_state": 0.94129132532661,
        "bes_terminal": 0.0002342516496232666,
        "tes_terminal": 0.1148135837909966,
        "h2s_terminal": 0.028189664219157903,
        "objective": 15796.994065942168,
    }
...
def test_load_debug_state():
    ...
    debug_state_path = (
        config_root / "mpcstate_2025-09-18 15:48:42-1758232122_step64.json"
    )
