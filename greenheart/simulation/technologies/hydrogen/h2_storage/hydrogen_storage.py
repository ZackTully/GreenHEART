...
class HydrogenStorage:
    def __init__(self, h2s_dict=None):
        ...
        if h2s_dict is None:
            ...
            self.max_capacity_kg = h2s_dict["max_capacity_kg"]
            self.min_capacity_kg = h2s_dict["min_capacity_kg"]
            self.max_charge_rate_kg_hr = h2s_dict["max_charge_rate_kgphr"]
            self.max_discharge_rate_kg_hr = self.max_charge_rate_kg_hr
            self.storage_state = h2s_dict["initial_state"]
        ...
        self.store_charge = np.zeros(sim_duration)
    ...
    def create_control_model(self):
        ...
        bounds_dict = {
            "u_lb": np.array([0, 0]),
            "u_ub": np.array(
                [self.max_charge_rate_kg_hr, self.max_discharge_rate_kg_hr]
            ),
            "x_lb": np.array([0]),
    ...
    def low_level_controller(self, available_massflow, desired_massflow):
        ...
        # Upper limits on charging
        # 1. charge rate
        # 2. cannot charge above max capacity
        # 3. cannot charge with more than is available
    ...
    def step(self, h2_input, dispatch, step_index):
        ...
        model_output, u_model, u_passthrough, u_curtail = self.low_level_controller(
            available_massflow, desired_massflow
        )
...
if __name__ == "__main__":
    ...
    P_avail_curtail = np.max(
        [np.zeros(len(t)), P_avail - P_des_pos - P_des_charge], axis=0
    )
    ...
    ax.fill_between(
        t, curtail_bottom, curtail_bottom + P_avail_curtail, label="curtail"
    )
